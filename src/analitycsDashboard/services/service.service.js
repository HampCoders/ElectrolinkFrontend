...
export class ServiceService {
    ...
    async getByTechnicianId(technicianId) {
        ...
        // Si data es un array de servicios, mapeamos a Service
        return Array.isArray(data) ? data.map(s => new Service(s)) : [];
    }

    async getRequestsByTechnicianId(technicianId) {
        const { data } = await httpInstance.get(`${import.meta.env.VITE_API_BASE_URL}/clients/${technicianId}/requests`);
        // data es un array de requests
